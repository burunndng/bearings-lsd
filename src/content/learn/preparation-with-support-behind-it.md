---
title: "Preparation, with the support behind it"
summary: "Set and setting research, and why a plan made ahead of time tends to hold up when in-the-moment thinking gets harder."
order: 20
lastReviewed: 2026-08-01
sources:
  - cite: "Haijen, E. et al. (2018). Predicting Responses to Psychedelics: A Prospective Study. Frontiers in Pharmacology."
    url: "https://doi.org/10.3389/fphar.2018.00897"
    year: 2018
  - cite: "Hartogsohn, I. (2016). Set and setting, psychedelics and the placebo response. Journal of Psychopharmacology."
    url: "https://doi.org/10.1177/0269881116677852"
    year: 2016
  - cite: "Watts, R., & Luoma, J. (2020). The use of the psychological flexibility model to support psychedelic assisted therapy. Journal of Contextual Behavioral Science."
    url: "https://doi.org/10.1016/j.jcbs.2019.12.004"
    year: 2020
  - cite: "Gollwitzer, P., & Sheeran, P. (2006). Implementation Intentions and Goal Achievement: A Meta-analysis of Effects and Processes. Advances in Experimental Social Psychology."
    url: "https://doi.org/10.1016/S0065-2601(06)38002-1"
    year: 2006
---

"Set and setting" — mindset going in, and the physical and social
environment around the experience — is one of the older ideas in this
field, and it holds up reasonably well against more recent research.
Studies looking at what predicts how an experience goes have found that
factors present before anything starts, like expectation and readiness,
correlate with people's later reports of how manageable an experience
felt. That is consistent with a placebo-adjacent mechanism: context and
expectation shape experience, which is a real effect, not a dismissal of
the experience itself.

Separately, a psychological flexibility framework — being willing to have
a difficult feeling rather than needing to control or avoid it — has been
proposed as one lens for supporting people through psychedelic-assisted
work. It is one model among several, not a settled consensus, but it maps
onto something practical: acceptance-based coping tends to serve people
better in the moment than the alternative of fighting what is happening.

None of this is about mindset controlling outcome. Research on
implementation intentions — plans made ahead of time in a specific
if-this-then-that structure — has found they help people carry through on
decisions made in a calmer state precisely because deliberation is harder
in the moment itself. That is a mechanism for why preparation done in
advance, not improvised in the moment, is the more reliable kind. See
[Deciding things early](/card/deciding-things-early) and the Sheet for
what that looks like in practice.
